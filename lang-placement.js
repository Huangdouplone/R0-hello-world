...
{q:"一个模块既要能被导入、又要能直接运行时，通常用什么来判断当前处于哪种情况？",o:["用 __name__ 是否等于 '__main__' 判断"," 用 sys.argv 的长度判断"," 用 os.getcwd() 判断"," 用 type(__file__) 判断"],a:0,why:"被导入时 __name__ 是模块名，直接运行时才是 '__main__'，据此把自测代码放进保护块。",st:6},
...
{q:"为什么带参数的宏建议把参数整体加括号？",o:["因为宏只做文本替换，运算符优先级会改变语义"," 因为不加括号无法编译"," 因为加括号能提升运行速度"," 因为这是编译器的强制要求"],a:0,why:"x*x 展开 SQR(1+2) 得到 1+2*1+2 = 5；写成 ((x)*(x)) 才会得到 9。",st:9},
...
{q:"下列关于引用的说法，正确的是？",o:["不存在「空引用」这种合法状态"," 引用可以先不初始化，以后再绑定"," 引用可以像指针一样做地址算术运算"," 引用必须用 new 创建"],a:0,why:"引用是对象的别名，必须初始化且终生只认一个对象，因此没有空引用。",st:2},
...
{q:"写了 override 但签名与基类虚函数不匹配时会发生什么？",o:["编译直接报错"," 运行时崩溃"," 静默变成普通重载"," 编译器自动改签名去匹配"],a:0,why:"override 的价值就是把「以为重写了其实没有」变成编译期错误，而不是留到运行期才发现。",st:6},
...
{q:"为什么静态方法里不能使用 this？",o:["静态方法属于类，调用时不依赖任何实例"," 静态方法的语法禁止 this 关键字"," 静态方法一定是私有的"," 静态方法执行得更快"],a:0,why:"this 指向调用对象，而静态方法属于类本身，可能根本没有任何实例存在。",st:5},
...
{q:"给实例方法加 synchronized 修饰，锁住的是哪个对象？",o:["调用该方法的那个实例（this）"," 该类的 Class 对象"," 方法内部的所有局部变量"," 整个堆内存"],a:0,why:"修饰实例方法锁 this，修饰静态方法才锁 Class 对象，这个区别在排查死锁时很关键。",st:9},
...
{q:"select 中同时有多个 case 就绪时，Go 会怎么处理？",o:["随机挑一个执行"," 按书写顺序挑第一个"," 依次全部执行"," 立即 panic"],a:0,why:"随机选择避免了对某个 channel 的隐式偏好，也让压力测试下的行为更公平。",st:5,d:5},
